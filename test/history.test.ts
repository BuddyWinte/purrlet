...
import { History, type HistoryCommand } from "../src/core/history";
...
const createAddCommand = (item: DocumentItem): HistoryCommand => ({
    execute: (document) => {
        document.add(item);
    },
...
describe("History", () => {
    ...
    test("executes a command", () => {
        ...
        history.execute(document, createAddCommand(item));
    ...
    test("new commands clear redo history", () => {
        ...
        history.execute(document, createAddCommand(first));

        history.undo(document);

        expect(history.canRedo()).toBe(true);

        history.execute(document, createAddCommand(second));
    ...
    test("multiple commands undo in reverse order", () => {
        ...
        history.execute(document, createAddCommand(third));
    ...
    test("clear removes all history", () => {
        ...
        history.execute(document, createAddCommand(createStroke("stroke-1")));
