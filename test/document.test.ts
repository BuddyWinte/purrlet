...
import type { DocStroke, DocumentItem } from "../src/types";

const createStroke = (id: string): DocumentItem => {
...
describe("Document", () => {
    ...
    test("begins a stroke", () => {
        ...
        const stroke = document.beginStroke("#ff0000", 10, 20, 5, "draw");
        ...
        expect(document.size).toBe(1);
        expect(document.getCurrentStroke()).toEqual(stroke);
    });

    test("begins an erasing stroke", () => {
        const document = new Document();

        const stroke = document.beginStroke("#000000", 10, 20, 5, "erase");
    ...
    test("adds points to the current stroke", () => {
        ...
        document.beginStroke("#000000", 10, 20, 5, "draw");

        expect(document.addPoint(20, 30, 10)).toBe(true);

        const stroke = document.getCurrentStroke();
    ...
    test("cannot add a point without an active stroke", () => {
        ...
        expect(document.addPoint(20, 30, 10)).toBe(false);
    ...
    test("ends the current stroke", () => {
        ...
        const original = document.beginStroke("#000000", 10, 20, 5, "draw");

        document.addPoint(20, 30, 10);
        ...
        expect(document.get(original.id)).toEqual({
            type: "stroke",
            data: {
                ...original,
                points: [
    ...
    test("removing the current stroke clears drawing state", () => {
        ...
        const stroke = document.beginStroke("#000000", 10, 20, 5, "draw");

        expect(document.isDrawing).toBe(true);

        expect(document.remove(stroke.id)).toBe(true);
    ...
    test("starts a new stroke by ending the previous stroke", () => {
        ...
        const first = document.beginStroke("#ff0000", 10, 20, 5, "draw");

        const second = document.beginStroke("#0000ff", 30, 40, 10, "draw");

        expect(document.size).toBe(2);
        expect(document.get(first.id)).toBeDefined();
        expect(document.get(second.id)).toBeDefined();
        expect(document.getCurrentStroke()).toEqual(second);
        expect(document.isDrawing).toBe(true);
    });
    ...
    test("getStrokes returns only stroke items", () => {
        ...
        expect(strokes[0]).toEqual(stroke.data);
    });
    ...
    test("getItems returns all document items", () => {
        ...
        expect(document.getItems()).toEqual([stroke, fill]);
    });
});
