...
export const eraserTool: Tool<EraserConfig> = {
    ...
    create(config: Readonly<EraserConfig> = {}): ToolInstance<EraserConfig> {
        let drawing = false;
        ...
        const sizeFor = (pointer: PurrletPointer): number => {
            ...
            const pressure = Math.max(0.05, pointer.pressure);
        ...
        return {
            ...
            onPointerMove(pointer: PurrletPointer, renderer: Renderer): void {
                ...
                renderer.setMode("erase");
                renderer.addPoint(pointer.x, pointer.y, sizeFor(pointer));
            },

            onPointerUp(_pointer: PurrletPointer, renderer: Renderer): void {
                if (!drawing) {
                    return;
                }
