...
export class Renderer {
  ...
  resize(width?: number, height?: number): void {
    ...
    const logicalWidth = Math.max(1, Math.round(width ?? rect.width));

    const logicalHeight = Math.max(1, Math.round(height ?? rect.height));
    ...
    if (sizeChanged || backingStoreChanged) {
      this.redraw();
    }
  }
  ...
  private floodFill(
    ...
    while (queue.length > 0) {
      ...
      for (let currentX = left; currentX <= right; currentX++) {
        ...
        visited[y * width + currentX] = 1;

        if (y > 0) {
          const aboveIndex = (y - 1) * width + currentX;

          if (visited[aboveIndex] === 0 && matches(currentX, y - 1)) {
        ...
        if (y < height - 1) {
          ...
          if (visited[belowIndex] === 0 && matches(currentX, y + 1)) {
            queue.push([currentX, y + 1]);
          }
        }
      }
    ...
    this.expandFillEdges(image, fill, target);
  }
  ...
  private expandFillEdges(
    ...
    const blendEdge = (x: number, y: number): void => {
      const index = (y * width + x) * 4;

      const currentR = original[index] ?? 0;

      const currentG = original[index + 1] ?? 0;

      const currentB = original[index + 2] ?? 0;

      const currentA = original[index + 3] ?? 0;
      ...
      const amount = 1 - targetDistance / maxDistance;

      data[index] = Math.round(currentR * (1 - amount) + fill[0] * amount);

      data[index + 1] = Math.round(currentG * (1 - amount) + fill[1] * amount);

      data[index + 2] = Math.round(currentB * (1 - amount) + fill[2] * amount);

      data[index + 3] = Math.round(currentA * (1 - amount) + fill[3] * amount);
    };
