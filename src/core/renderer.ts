...
export class Renderer {
    ...
    beginStroke(color: string, size: number, x: number, y: number): void {
        ...
        this.ctx.globalCompositeOperation =
            this.activeStroke.compositeOperation;
    ...
    private expandFillEdges(
        ...
        const blendEdge = (x: number, y: number): void => {
            ...
            data[index] = Math.round(
                currentR * (1 - amount) + fill[0] * amount,
            );

            data[index + 1] = Math.round(
                currentG * (1 - amount) + fill[1] * amount,
            );

            data[index + 2] = Math.round(
                currentB * (1 - amount) + fill[2] * amount,
            );

            data[index + 3] = Math.round(
                currentA * (1 - amount) + fill[3] * amount,
            );
        };
    ...
    private renderRectangle(rectangle: DocRectangle): void {
        ...
        this.ctx.globalAlpha = clamp(
            normalizeNumber(rectangle.opacity, 1),
            0,
            1,
        );
