...
export interface PointerHandlers {
    readonly down: (pointer: PurrletPointer, event: PointerEvent) => void;

    readonly move: (pointer: PurrletPointer, event: PointerEvent) => void;

    readonly up: (pointer: PurrletPointer, event: PointerEvent) => void;

    readonly cancel?: (pointer: PurrletPointer, event: PointerEvent) => void;
}

const pointerTypes: readonly PointerType[] = ["mouse", "pen", "touch"];

const isPointerType = (value: string): value is PointerType =>
...
const normalizeTilt = (tilt: number): number => {
...
export function bindPointer(
    ...
    const getPoint = (event: PointerEvent): PurrletPointer => {
    ...
    const handlePointerDown = (event: PointerEvent): void => {
        ...
        handlers.down(getPoint(event), event);
    };

    const handlePointerMove = (event: PointerEvent): void => {
        handlers.move(getPoint(event), event);
    };

    const handlePointerUp = (event: PointerEvent): void => {
        handlers.up(getPoint(event), event);
    ...
    const handlePointerCancel = (event: PointerEvent): void => {
        handlers.cancel?.(getPoint(event), event);
