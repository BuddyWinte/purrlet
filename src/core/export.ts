...
const normalizeQuality = (quality: number | undefined): number | undefined => {
...
export function canvasToBlob(
    ...
    return canvasToBlobInternal(canvas, type, normalizeQuality(quality));
}
...
export function canvasToDataURL(
    ...
    return canvas.toDataURL(type, normalizeQuality(quality));
}
...
export async function exportCanvas(
    ...
    if (scale === 1 && options.background === undefined) {
        return canvasToBlobInternal(canvas, mimeType, quality);
    }
    ...
    if (context === null) {
        throw new Error("[Purrlet] Export rendering context unavailable.");
    }
    ...
    try {
        return await canvasToBlobInternal(temporaryCanvas, mimeType, quality);
    } finally {
